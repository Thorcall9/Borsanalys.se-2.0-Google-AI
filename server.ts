...
async function startServer() {
  ...
  /**
   * --- VOTING SYSTEM (NEWSLETTER) ---
   */

  // Public: Catch vote from email link
  app.get("/vote", async (req, res) => {
    ...
    try {
      ...
      res.status(500).send("Ett oväntat fel uppstod vid röstningen. Försök igen senare.");
    }
  });

  // Admin: Get vote counts per stock
  app.get("/api/admin/votes", async (req, res) => {
    try {
      ...
      console.error('Admin Votes Fetch Error:', error.message);
      res.status(500).json({ error: "Kunde inte hämta röstresultat." });
    }
  });
