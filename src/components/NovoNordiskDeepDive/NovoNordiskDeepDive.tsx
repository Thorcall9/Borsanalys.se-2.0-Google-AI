...
<div style={{fontFamily:"‘Georgia’,‘Times New Roman’,serif",background:T.bg,minHeight:"100vh",color:T.ink,opacity:mounted?1:0,transition:"opacity 0.45s ease"}}>
  ...
  <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"36px 40px 0"}}>
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:20,marginBottom:24,opacity:mounted?1:0,transform:mounted?"none":"translateY(10px)",transition:"all 0.5s ease 0.1s"}}>
      <div style={{display:"flex",gap:18,alignItems:"flex-start"}}>
        <div style={{width:52,height:52,borderRadius:14,background:`linear-gradient(135deg,${T.accent},#003BB5)`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:900,fontSize:18,letterSpacing:-1,boxShadow:`0 4px 16px ${T.accent}44`,flexShrink:0}}>NN</div>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:6}}>
            <h1 style={{margin:0,fontSize:24,fontWeight:900,color:T.ink,letterSpacing:-0.5,fontFamily:"Georgia,serif"}}>Novo Nordisk A/S</h1>
            <Tag>NOVO-B · NVO</Tag>
            <KopBadge/>
          </div>
          <p style={{margin:0,color:T.sub,fontSize:13.5}}>Läkemedel · Diabetes & Fetma · Nasdaq Köpenhamn & NYSE</p>
          <p style={{margin:"4px 0 0",color:T.muted,fontSize:12}}>Analys av Carl Fredrik Thor · 23 mars 2026 · ISIN: DK0060534915</p>
        </div>
      </div>
      <div style={{background:T.bg,border:`1.5px solid ${T.border}`,borderRadius:16,padding:"16px 24px",textAlign:"center",flexShrink:0,opacity:mounted?1:0,transform:mounted?"none":"scale(0.95)",transition:"all 0.5s ease 0.2s"}}>
        <div style={{fontSize:40,fontWeight:900,color:T.accent,lineHeight:1,fontFamily:"Georgia,serif"}}>29</div>
        <div style={{fontSize:11,color:T.muted,fontWeight:700,letterSpacing:0.5,marginTop:2}}>/ 40 POÄNG</div>
        <div style={{fontSize:12,color:T.gold,fontWeight:700,marginTop:6}}>★★★★ 72,5%</div>
      </div>
    </div>

    {/* Score pills */}
    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:0,opacity:mounted?1:0,transition:"opacity 0.5s ease 0.3s"}}>
      {allScores.map(({key,val,max,inv})=>{
        const pct=val/max;
        const color=inv?(pct>=0.6?T.green:pct>=0.4?T.gold:T.red):(pct>=0.8?T.green:pct>=0.6?T.accent:pct>=0.4?T.gold:T.red);
        return <ScorePill key={key} label={key} val={val} max={max} color={color}/>;
      })}
    </div>
          ...
          <ResponsiveContainer width="100%" height={170}>
            <LineChart data={epsData} margin={{top:4,right:16,bottom:0,left:0}}>
              ...
              <Line type="monotone" dataKey="v" name="EPS" stroke={T.accent} strokeWidth={2.5}
                dot={(props)=>{const{cx,cy,payload}=props;return<circle key={cx} cx={cx} cy={cy} r={4} fill={payload.e?"transparent":T.accent} stroke={T.accent} strokeWidth={2} strokeDasharray={payload.e?"4 2":"0"}/>;}}
              />
            </LineChart>
          </ResponsiveContainer>
            ...
            {[
              ...
              <div key={title} style={{background:bg,border:`1.5px solid ${color}22`,borderRadius:12,padding:16}}>
                ...
                <ul style={{margin:0,paddingLeft:16}}>
                  {items.map((item,i)=><li key={i} style={{color:T.sub,fontSize:13,marginBottom:6,lineHeight:1.6}}>{item}</li>)}
                </ul>
              </div>
            ))}
          </div>
