...
function MetaRevenueFlow() {
  ...
  const resultNode = (x: number, y: number, title: string, value: string, percent: string, tone: string, key: string) => <g key={key}><rect x={x} y={y} width="150" height="52" rx="6" fill="#fffdf9" stroke="#d9d3c6"/><text x={x + 75} y={y + 18} textAnchor="middle" fill="#24332e" fontSize="10" fontWeight="800">{title}</text><text x={x + 75} y={y + 34} textAnchor="middle" fill={tone} fontSize="11" fontWeight="900">{value}</text><text x={x + 75} y={y + 46} textAnchor="middle" fill="#7b827d" fontSize="9" fontWeight="700">{percent}</text></g>;
  return <figure className="mb-10 overflow-hidden rounded-[2rem] border border-border/60 bg-card shadow-xl shadow-black/5">
    ...
    <div className="overflow-x-auto premium-scrollbar bg-[#f8f0e2]"><svg data-testid="meta-revenue-sankey" aria-label="Meta-intäktsflöde Q2 2026" className="w-full min-w-[980px] p-5 md:p-8" viewBox="0 0 1280 570" role="img">
      ...
      <path d="M224 160 C276 160 318 160 368 160 L368 396 C318 396 276 396 224 396 Z" fill="url(#meta-revenue-flow)"/><path d="M224 400 C276 400 318 396 368 396 L368 400 C318 400 276 404 224 404 Z" fill="#81b9ff" opacity=".9"/><path d="M224 408 C276 408 318 400 368 400 L368 403 C318 403 276 411 224 411 Z" fill="#bed9ff" opacity=".95"/>
      <path d="M383 160 C500 160 590 160 707 160 L707 254 C590 254 500 254 383 254 Z" fill="url(#meta-profit-flow)"/><path d="M383 254 C500 254 590 254 707 254 L707 403 C590 403 500 403 383 403 Z" fill="url(#meta-cost-flow)"/>
      <path d="M722 160 C790 160 830 190 888 190 L888 265 C830 265 790 235 722 235 Z" fill="url(#meta-profit-flow)"/><path d="M722 235 C790 235 830 330 888 330 L888 349 C830 349 790 254 722 254 Z" fill="url(#meta-cost-flow)"/>
      <path d="M903 190 C970 190 1008 190 1066 190 L1066 253 C1008 253 970 253 903 253 Z" fill="url(#meta-profit-flow)"/><path d="M903 253 C970 253 1008 330 1066 330 L1066 342 C1008 342 970 265 903 265 Z" fill="url(#meta-cost-flow)"/>
      {bar(209, 160, 236, "#2757b5", "advertising-bar")}{bar(209, 400, 4, "#69a8ff", "foa-other-bar")}{bar(209, 408, 3, "#9dc6ff", "rl-revenue-bar")}
      {bar(368, 160, 243, "#159252", "revenue-bar")}{bar(707, 160, 94, "#158e4e", "foa-profit-bar")}{bar(707, 254, 149, "#cf4036", "foa-cost-bar")}{bar(888, 190, 75, "#159252", "ebit-bar")}{bar(888, 330, 19, "#cf4036", "rl-loss-bar")}{bar(1066, 190, 63, "#159252", "net-income-bar")}{bar(1066, 330, 12, "#cf4036", "finance-tax-bar")}
      {label(195, 210, "Advertising", "59,363 md USD · 97,6 %", "#2757b5", "advertising-label", "end")}{label(195, 432, "FoA other revenue", "1,007 md USD · 1,7 %", "#4784d7", "foa-other-label", "end")}{label(195, 474, "Reality Labs revenue", "0,431 md USD · 0,7 %", "#4784d7", "rl-revenue-label", "end")}
      {resultNode(286, 256, "Omsättning", "60,801 md USD", "100,0 %", "#159252", "revenue-node")}{resultNode(535, 174, "FoA operating profit", "23,394 md USD", "38,5 %", "#159252", "foa-profit-node")}{resultNode(535, 330, "Övriga FoA-kostnader*", "36,976 md USD", "60,8 %", "#cf4036", "foa-cost-node")}{resultNode(778, 202, "Koncernens EBIT", "18,775 md USD", "30,9 %", "#159252", "ebit-node")}{resultNode(778, 347, "Reality Labs-förlust", "4,619 md USD", "7,6 %", "#cf4036", "rl-loss-node")}{resultNode(1080, 202, "Nettoresultat", "15,848 md USD", "26,1 %", "#159252", "net-income-node")}{resultNode(1008, 355, "Finans, skatt & övrigt*", "2,927 md USD", "4,8 %", "#cf4036", "finance-tax-node")}
